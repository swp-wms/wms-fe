...
export const getImportWeight = async () => {
    try {
        const response = await axios.get(`${api.GET_IMPORT_WEIGHT}`, {
            headers: {
...
export const getExportWeight = async () => {
    try {
        const response = await axios.get(`${api.GET_EXPORT_WEIGHT}`, {
            headers: {
                Authorization: `Bearer ${accessToken}`,
            },
        });
...
export const getImportOrder = async () => {
  try {
...
export const getExportOrder = async () => {
  try {
...
const fetchDeliveryDetails = async (id) => {
  try {
    const response = await axios.get(
      `${api.GET_DELIVERY_DETAIL_FOR_UPDATE_ORDER}/${id}`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
